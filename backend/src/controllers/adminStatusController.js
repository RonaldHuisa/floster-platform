    ...
    const [
      ...
      pool.query(`
        SELECT
          ...
          COALESCE(SUM(amount_to_receive) FILTER (WHERE status = 'pending'), 0) AS pending_to_pay_usdt,
          COALESCE(SUM(amount_to_receive) FILTER (WHERE status = 'paid'), 0) AS paid_withdrawals_usdt,
          COALESCE(SUM(amount_requested) FILTER (WHERE status = 'pending'), 0) AS pending_requested_usdt,
          COALESCE(SUM(fee_amount) FILTER (WHERE status = 'pending'), 0) AS pending_fees_usdt,
